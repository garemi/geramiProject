<template>
  <div
    :class="step === 1 ? 'background-black' : 'gray-background'"
    class="expand-viewport-min-height"
  >
    ...
    />
    <v-container fluid v-else-if="step === 2" class="flex flex-col gap-7">
      <div class="d-flex align-center" cols="12">
        <div class="div-size rounded-lg background-gray">
          <v-icon @click="step = 1"> mdi-chevron-right</v-icon>
        </div>
        <h3 class="mx-4 md-text">انتخاب دسته بندی</h3>
      </div>
      <div class="flex items-center gap-11">
        <div class="flex items-center gap-1 text-xs !grow-0">
          <input id="gold" value="gold" type="radio" v-model="categoryType" />
          <Label for="gold">طلا</Label>
        </div>
        <div class="flex items-center gap-1 text-xs !grow-0">
          <input id="coin" value="coin" type="radio" v-model="categoryType" />
      ...
      <div v-if="categoryType == 'gold'">
        <input
          ...
          v-model="goldSearch"
          type="search"
          class="w-full outline-none mb-7"
        />
        <div
          v-for="cate in categories.data.gold.filter((item) =>
            item.title.includes(goldSearch),
          )"
          :key="cate.id"
      ...
      <div v-if="categoryType == 'coin'">
        ...
        />
        <div
          v-for="cate in categories.data.coin.filter((item) =>
            item.title.includes(coinSearch),
          )"
          :key="cate.id"
          class="flex justify-between items-center py-4 border-b border-[rgba(0, 0, 0, 0.05)]"
          @click="
    ...
    <v-container class="" fluid v-else-if="step === 3">
      <!-- step 2 -->
      <v-form ref="form">
  ...
  export default {
    ...
    methods: {
      ...
      async submit() {
        ...
        formData.append("is_getting_offers", this.proposal);
        ...
        if (this.suggestedPrice) {
          formData.append("suggested_price", this.suggestedPrice);
        }
